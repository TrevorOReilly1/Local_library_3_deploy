# add 3 imports for API processing of Author's books and images
import requests
from django.conf import settings
from django.core.files.base import ContentFile
# add 3 imports for image processing
...
class BookCreate(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        uploaded_image = form.cleaned_data.get('book_image')
...
def author_get_books_api(request, pk):#Use the Google Books API to get a list of books for an author
    my_api_key = settings.GOOGLEBOOKS_API_KEY
    ...
    if (authorInfo_json['totalItems']==0):
        ...
        for b in authorInfo_json['items']:
            try: #if any exceptions occur, the book will not be added
                ...
                book,created = Book.objects.get_or_create(title=title, author=author,isbn=isbn, summary=summary)
...
def bookImageFromAPI(book, form):
    ...
    URL = f"https://www.googleapis.com/books/v1/volumes?q=isbn:{form.cleaned_data['isbn']}&key={my_api_key}"
    ...
    if bookInfo_json.get('totalItems', 0) != 0:
        api_title = bookInfo_json['items'][0]['volumeInfo'].get('title', '')

        if api_title.lower() == book.title.lower():
            imageURL = (
                bookInfo_json['items'][0]['volumeInfo']
                .get('imageLinks', {})
                .get('smallThumbnail')
            )
            ...
            if imageURL:
                response = requests.get(imageURL)
                if response.status_code == 200 and validateImage(response.content):
                    book.book_image.save(file_name, ContentFile(response.content), save=True)
                    imageFound = True
